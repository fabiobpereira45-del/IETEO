...
function mapAttendance(row: any): Attendance { return { id: row.id, studentId: row.student_id, disciplineId: row.discipline_id, date: row.date ? row.date.split('T')[0] : '', isPresent: row.is_present, createdAt: row.created_at } }
